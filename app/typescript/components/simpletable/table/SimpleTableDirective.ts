...
angular.module('simpletable.table', [])
    .directive('stTable', ['$timeout', 'SimpleTablePluginFactory', function($timeout, SimpleTablePluginFactory) {
        ...
        return {
            ...
            "  </thead>" +
            "  <tbody>" +
            "    <tr ng-click='simpleTable.onRowClicked($event, row)' ng-class='{selected: simpleTable.selection.isRowSelected(row)}' ng-repeat='row in tableData | filter:tableConfig.filter' >" +
